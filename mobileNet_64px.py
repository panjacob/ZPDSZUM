...
from utilis.statistics import make_plots, make_confusion_matrix, save_model

RESULT_FILENAME = "MobilNet_64px_3_64px_095training"  # nazwa pliku gdzie zostana zapisane wyniki w "files/results/{RESULT_FILENAME}"
test_model = "MobilNet_3_64px 095_training, steps=100, epoch=20, loss=sparse_categorical_crossentropy, optimizer=adamax=0.0001"  # Dodawany do wykresow
...
train_generator, test_generator = get_generators_augumented(dataset_name="Data_08", train_size=0.8, image_width=64)
destination_path = os.path.join('files', 'results', RESULT_FILENAME)
model, callbacks = get_model()
if LEARN_MODEL_TRUE_OR_LOAD_FALSE:
    history = model.fit(train_generator, epochs=20, steps_per_epoch=100,
